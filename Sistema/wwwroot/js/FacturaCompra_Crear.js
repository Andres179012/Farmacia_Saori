...
$(document).ready(function () {
        ...
        success: function (data) {
            ...
            if (data.data != null) {
                $.each(data.data, function (i, item) {
                    ...
                    if (item.estado == true) {
                        $("<option>").attr({ "value": item.id_Categoria }).text(item.nombre_Laboratorio).appendTo("#cboLaboratorio");
                    }
                })
                $("#cboLaboratorio").val($("#cboLaboratorio option:first").val());
            }
        ...
        success: function (data) {
            ...
            if (data.data != null) {
                $.each(data.data, function (i, item) {
                    ...
                    if (item.estado == true) {
                        $("<option>").attr({ "value": item.id_Proveedor }).text(item.proveedor).appendTo("#cboProveedor");
                    }
                })
    ...
    jQuery.ajax({
        url: "/FormasFarmaceuticas/Obtener",
        type: "GET",
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {

            $("#cboFormaFarmaceutica").html("");

            if (data.data != null) {
                $.each(data.data, function (i, item) {

                    if (item.estado == true) {
                        $("<option>").attr({ "value": item.id_FormaFarmaceutica }).text(item.forma_Faumaceutica).appendTo("#cboFormaFarmaceutica");
                    }
                })
                $("#cboFormaFarmaceutica").val($("#cboFormaFarmaceutica option:first").val());
            }
        ...
        success: function (data) {
            ...
            if (data.data != null) {
                $.each(data.data, function (i, item) {
                    ...
                    if (item.estado == true) {
                        $("<option>").attr({ "value": item.id_FormaPago }).text(item.forma_Pago).appendTo("#cboFormaPago");
                    }
                })
        ...
        "columns": [
            { "data": "id_FacturaCompra" },
            {
                "data": "objlaboratorio", render: function (data) {
                    return data.nombre_Laboratorio
                }
            },
            {
                "data": "objproveedores", render: function (data) {
            ...
            {
                "data": "id_FacturaCompra", "render": function (data, type, row, meta) {
    ...
    tabladata = $('#tbdata').DataTable({
        ...
        "columns": [
            ...
            {
                "data": "objfarmacos", render: function (data) {
                    return data.nombre_Generico
                }
            },
            {
                "data": "objformafarmaceutica", render: function (data) {
                    return data.forma_Faumaceutica
                }
            },
            {
                "data": "objformapago", render: function (data) {
            ...
            {
                "data": "id_DetalleCompra", "render": function (data, type, row, meta) {
                    return "<button class='text-primary bg - light - primary border - 0' type='button' onclick='abrirPopUpForm(" + JSON.stringify(row) + ")'><i class='bx bxs-edit'></i></button>" +
                        "<button class='ms-4 text-danger bg-light-danger border-0' type='button' onclick='eliminar(" + data + ")'><i class='bx bxs-trash'></i></button>"
                },
                "orderable": false,
